...
需要 Node.js 22.13+，首次在项目目录运行 `npm install`。当前工作区已安装依赖。

```bash
cd /home/sfwh/Desktop/ppt/palette-studio
...
```bash
codex mcp add palette-ppt -- node /home/sfwh/Desktop/ppt/palette-studio/cli/mcp.mjs
```
...
```json
{
  "mcpServers": {
    "palette-ppt": {
      "command": "node",
      "args": ["/home/sfwh/Desktop/ppt/palette-studio/cli/mcp.mjs"]
    }
  }
}
```
...
```bash
node /home/sfwh/Desktop/ppt/palette-studio/cli/palette-ppt.mjs install-skill
```
...
本次只提供安装器，没有修改用户的 MCP 配置或技能目录。技能格式参考 [官方技能文档](https://developers.openai.com/codex/skills)。
