...
const schema=z.object({version:z.literal(1).optional(),name:z.string().min(1).max(50).optional(),colors:z.object({background:z.string().regex(/^#[0-9a-f]{6}$/i),accent:z.string().regex(/^#[0-9a-f]{6}$/i),text:z.string().regex(/^#[0-9a-f]{6}$/i)}).strict(),title:z.string().max(40).optional(),subtitle:z.string().max(60).optional()}).strict();
