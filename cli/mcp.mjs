...
server.registerTool('generate_ppt',{description:'在运行本 MCP 服务的本机生成 10 页可编辑 PPT。需要用户指定的绝对输出路径，默认不覆盖已有文件。',inputSchema:{config:schema,outputPath:z.string(),overwrite:z.boolean().default(false)},annotations:{readOnlyHint:false,destructiveHint:true,openWorldHint:false}},async({config,outputPath,overwrite})=>{try{if(!path.isAbsolute(outputPath))throw new Error('outputPath 必须是绝对路径');return result(await writePresentation(normalizeConfig(config),outputPath,overwrite));}catch(e){return {isError:true,content:[{type:'text',text:e.message}]};}});
