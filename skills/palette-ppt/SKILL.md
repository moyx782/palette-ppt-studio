---
name: palette-ppt
description: 从配色 JSON 生成十页可编辑 PPT 模板（含图片占位页），可选极简留白、大胆撞色、编辑杂志、网格分割四种版式风格，验证颜色配置，并与色页网页导入格式保持一致。用户要求用 JSON 配色制作 PPT 模板时使用。
---
...
- 当前输出固定十页：封面、章节、内容、数据、全幅图片、左文右图、双图对比、大引言、团队、结束页，十页共用所选 style 的版式语言。图片页内含虚线「图片占位」框，提示用户下载模板后自行替换为真实图片；不要替用户编造图片。数据页为示例数据，不将其描述为用户的真实业务数据。网页预览和 PPT 字体、装饰图形可能略有差异。
